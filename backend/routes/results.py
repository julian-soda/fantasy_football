...
def _lookup_key(provider: str, league_id: str, year: int, through_week: int | None) -> str:
    return f"idx#{provider}#{league_id}#{year}#{through_week or 'full'}"


def find_cached_result(provider: str, league_id: str, year: int, through_week: int | None) -> str | None:
    """Return the result_id for a previously computed result, or None."""
    resp = _table().get_item(Key={"result_id": _lookup_key(provider, league_id, year, through_week)})
    item = resp.get("Item")
    return item["target_id"] if item else None


def save_result(provider: str, league_id: str, year: int, through_week: int | None, team_events: list) -> str:
    ...
    _table().put_item(Item={
        "result_id": _lookup_key(provider, league_id, year, through_week),
        "target_id": result_id,
    })
    return result_id
...
def lookup_result(league_id: str, year: int, through_week: int | None = None, provider: str = "yahoo"):
    """Check if a cached result exists for this league/year/through_week combo."""
    result_id = find_cached_result(provider, league_id, year, through_week)
    if not result_id:
