...
def fetch_stats(league_id: str, year: int, env_file: str = '.env', access_token_json: dict = None,
    ...
    if access_token_json is not None:
        ...
        return _do_fetch(league_id, year, token_json=token_dict,
                         through_week=through_week, debug=debug)
    else:
...
def _do_fetch(league_id, year, env_dir=None, token_json=None,
    ...
    yfpy v15 changed its auth API: env_file_location/yahoo_access_token_json are
    gone. It now reads credentials + tokens from token.json in auth_dir. For the
    webapp path we write a temp token.json; for the CLI path we use env_dir so
    yfpy persists the token there across runs.
    """
    tmp = None
    try:
        if token_json is not None:
            # Webapp path: write token to a temp dir so yfpy can read it.
            tmp = tempfile.TemporaryDirectory()
            auth_dir = Path(tmp.name)
            (auth_dir / "token.json").write_text(json.dumps(token_json))
            consumer_key = token_json['consumer_key']
            consumer_secret = token_json['consumer_secret']
        else:
            # CLI path: persist token.json alongside .env for reuse across runs.
            auth_dir = env_dir
            consumer_key = os.environ.get('YAHOO_CONSUMER_KEY')
            consumer_secret = os.environ.get('YAHOO_CONSUMER_SECRET')

        # First query (no game_id) resolves the year → game_id.
